...
//            $totalTVA += round($prixTTC - $prixHT,2);
    ...
    public function validationCommandeAction($id)
    {
             ...
             $ligneCommandes=$em->getRepository('EcommerceBundle:LigneCommande')->findBy(array('panier' => $bdPanier->getId()));
                 if($ligneCommandes!=null){ 
                     foreach ($ligneCommandes as $ligneCommandes){
                        $produit= $em->getRepository('EcommerceBundle:Produits')->find($ligneCommandes->getProduit()->getId());
                        $produit->setNbVente($produit->getNbVente()+$ligneCommandes->getQte());
                        $produit->setQuantite($produit->getQuantite()-$ligneCommandes->getQte());
                        $em->persist($produit);
                        $em->remove($ligneCommandes);
                        $em->flush();
        ...
        $message = \Swift_Message::newInstance()
                ...
                ->setFrom(array('maher.esprit@gmail.tn' => "E-shop"))
